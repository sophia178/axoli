...
export function LoadingScreen({ visible = true }: { visible?: boolean }) {
  ...
  const facts = useMemo(() => {
    const list: string[] = []
    for (let i = 1; i <= 50; i++) list.push(tLoading(`facts.${i}`))
    return list
  }, [tLoading])
  ...
  const bubble = useMemo(() => {
    return (
      ...
      >
        <div className="font-heading text-lg text-gold">{tLoading('didYouKnow')}</div>
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex min-h-screen items-center justify-center bg-bg">
      <div className="absolute left-1/2 top-10 -translate-x-1/2">
        <div className="font-heading text-2xl text-pink">{tCommon('appName')}</div>
      </div>
      <div className="mx-auto flex w-full max-w-2xl flex-col items-center px-6">
        <div className="mb-8 w-full">{bubble}</div>
        <AxolotlBounce ariaLabel={tLoading('loadingAxolotlAria')} />
      </div>
    </div>
  )
}
